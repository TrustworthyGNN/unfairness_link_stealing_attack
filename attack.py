...
def plot_g_auc(g_auc_dict,label_num, desc="similarity score"):
    ...
    for key in g_auc_dict:
        harvest[class_in_prediction.index(int(key[0])), class_in_prediction.index(int(key[1]))] = round(g_auc_dict[key] * 100, 2)
        if key[0] != key[1]:
            harvest[class_in_prediction.index(int(key[1])), class_in_prediction.index(int(key[0]))] = round(g_auc_dict[key] * 100, 2)
    ...
    with open("result/attack_0_evaluation_group_at_%s.txt" % timestamp, "a") as wf:
        ...
        for i in range(len(sim_list_str)):
            ...
            for key in group_dict:
                ...
                print(sim_list_str[i], key, 'group_size: ', len(group_dict[key]), 'acc_s: ', round(g_acc_s*100,2), 'acc_g: ', round(g_acc_g*100,2))
                wf.write(
            ...
            print(sim_list_str[i] + " group-based-ave-acc ", round(group_based_ave_acc*100,2), " single-based-ave-acc ", round(single_based_ave_acc*100,2))
...
def viz(pred_prob_list, label, group_dict, node_label_num):
    ...
    for i in range(len(sim_list_str)):
        ...
        group_based_ave_acc = np.inner(acc_group_threshold, group_size) / group_size.sum()
        single_based_ave_acc = np.inner(acc_single_threshold, group_size) / group_size.sum()
        print(sim_list_str[i] + " single-based-ave-acc " + str(round(single_based_ave_acc*100,2)) + " group-based-ave-acc " + str(round(group_based_ave_acc*100,2)) )
